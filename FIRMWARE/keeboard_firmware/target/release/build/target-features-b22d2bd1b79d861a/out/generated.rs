...
pub const CURRENT_TARGET: Target = {
    ...
    let target = if let Ok(feature) = Feature::new(arch, "cmpxchg16b") { target.with_feature(feature) } else { target };
    let target = if let Ok(feature) = Feature::new(arch, "fxsr") { target.with_feature(feature) } else { target };
    let target = if let Ok(feature) = Feature::new(arch, "lahfsahf") { target.with_feature(feature) } else { target };
    ...
    let target = if let Ok(feature) = Feature::new(arch, "sse3") { target.with_feature(feature) } else { target };
    let target = if let Ok(feature) = Feature::new(arch, "x87") { target.with_feature(feature) } else { target };
    target
};
